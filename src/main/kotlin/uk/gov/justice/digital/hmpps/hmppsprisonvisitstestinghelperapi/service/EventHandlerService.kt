...
import uk.gov.justice.digital.hmpps.hmppsprisonvisitstestinghelperapi.dto.MessageAttribute
import uk.gov.justice.digital.hmpps.hmppsprisonvisitstestinghelperapi.dto.MessageAttributes
...
class EventHandlerService(
  ...
  fun handleNonAssociationEvent(nonAssociationEventDto: NonAssociationEventDto) {
    ...
    val domainEventJson = objectMapper.writeValueAsString(DomainEvent(Events.NON_ASSOCIATION_CREATE_EVENT.eventType, values))

    sqsService.sendDomainEvent(buildSqsMessage(Events.NON_ASSOCIATION_CREATE_EVENT.eventType, domainEventJson))
  ...
  fun handlePrisonerReleaseEvent(prisonerReleasedNotificationDto: PrisonerReleasedNotificationDto) {
    ...
    val domainEventJson = objectMapper.writeValueAsString(DomainEvent(Events.PRISONER_RELEASE_EVENT.eventType, values))

    sqsService.sendDomainEvent(buildSqsMessage(Events.PRISONER_RELEASE_EVENT.eventType, domainEventJson))
  ...
  fun handlePrisonerReceivedEvent(prisonerEventDto: PrisonerReceivedEventDto) {
    ...
    val domainEventJson = objectMapper.writeValueAsString(DomainEvent(Events.PRISONER_RECEIVE_EVENT.eventType, values))

    sqsService.sendDomainEvent(buildSqsMessage(Events.PRISONER_RECEIVE_EVENT.eventType, domainEventJson))
  ...
  fun handlePrisonerRestrictionChangeEvent(prisonerRestrictionEventDto: PrisonerRestrictionEventDto) {
    ...
    val domainEventJson = objectMapper.writeValueAsString(DomainEvent(Events.PRISONER_RESTRICTION_CHANGE_EVENT.eventType, values))

    sqsService.sendDomainEvent(buildSqsMessage(Events.PRISONER_RESTRICTION_CHANGE_EVENT.eventType, domainEventJson))
  ...
  fun handlePrisonerAlertCreatedUpdatedEvent(prisonerAlertCreatedUpdatedEventDto: PrisonerAlertCreatedUpdatedEventDto) {
    ...
    val domainEventJson = objectMapper.writeValueAsString(DomainEvent(Events.PRISONER_ALERT_UPDATED_EVENT.eventType, values))

    sqsService.sendDomainEvent(buildSqsMessage(Events.PRISONER_ALERT_UPDATED_EVENT.eventType, domainEventJson))
  ...
  fun handleVisitorRestrictionChangeEvent(visitorRestrictionEventDto: VisitorRestrictionEventDto) {
    ...
    val domainEventJson = objectMapper.writeValueAsString(DomainEvent(Events.VISITOR_RESTRICTION_CHANGE_EVENT.eventType, values))

    sqsService.sendDomainEvent(buildSqsMessage(Events.VISITOR_RESTRICTION_CHANGE_EVENT.eventType, domainEventJson))

    LOG.info("processed visitor restriction change event with details - $visitorRestrictionEventDto")
  }

  private fun buildSqsMessage(eventType: String, domainEventJson: String): SQSMessage {
    val attrs = MessageAttributes().apply {
      this["eventType"] = MessageAttribute(type = "String", value = eventType)
    }
